<script lang="ts">
  import { CheckCircle, CircleNotch, Copy, Eye, EyeSlash, FloppyDisk, Lock } from "phosphor-svelte";
  ...
  let title = $derived(
    formStep === 'webhook' ? "Configure Webhook" : (editingAccount ? "Update Provider" : "Add Payment Provider")
  );
  ...
  async function save() {
    ...
    try {
      ...
      if (editingAccount) {
        await apiFetch(`/api/dashboard/providers/accounts/${editingAccount.id}`, {
          method: "PATCH",
          body: JSON.stringify({
...
<SidePanel {open} {title} onclose={onClose} width="max-w-md">
  <div class="p-6 space-y-8">
    {#if providerError}
      <div class="p-4 bg-error-bg border border-error text-error text-xs flex items-center gap-2">
        {providerError}
      </div>
    {/if}
        ...
        {#each selectedProviderConfig.fields as field (field.key)}
          <div>
            ...
            <div class="relative input-icon-wrapper">
              ...
              />
              <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-text-dim hover:text-text-primary" onclick={() => showSecretFields[field.key] = !showSecretFields[field.key]}>
    ...
    {:else if formStep === 'webhook' && lastCreatedProviderId}
      <div class="space-y-4">
        ...
        <div class="bg-[var(--color-bg-code)] p-3 rounded border border-border flex gap-2">
          ...
          <button class="text-text-dim hover:text-text-primary" onclick={() => copyUrl(`${apiBase}/webhooks/${projectId}/${lastCreatedProviderId}`)}><Copy size={14} weight="fill" /></button>
        </div>
      </div>
    {/if}
  </div>
  <div class="p-6 border-t border-border flex justify-end gap-3 sticky bottom-0 bg-bg-card">
    {#if formStep === 'configure'}
      ...
      <button class="btn btn-primary flex items-center gap-2" onclick={save} disabled={isSavingProvider}>
        {#if isSavingProvider} <CircleNotch size={14} class="animate-spin" weight="duotone" /> Saving... {:else} <FloppyDisk size={14} weight="fill" /> {editingAccount ? 'Update' : 'Save'} Provider {/if}
      </button>
    {:else}
...
<style>
  ...
  :global(.input.input-has-icon-left) {
    padding-left: 2.5rem;
  }
