<script lang="ts">
  import { ArrowRight, CircleNotch, Envelope, Lock } from "phosphor-svelte";
  ...
  >
    <div class="w-full max-w-sm">
      ...
      <form class="flex flex-col gap-4" onsubmit={handleSubmit}>
        <div>
          <label for="email" class="block text-[10px] font-bold text-text-dim uppercase tracking-widest mb-2">Email</label>
          <div class="input-icon-wrapper">
            <Envelope size={18} class="input-icon-left" weight="duotone" />
            <input type="email" id="email" bind:value={email} placeholder="you@example.com" required class="input input-has-icon-left" />
          </div>
        </div>

        <div>
          <div class="flex items-center justify-between mb-2">
            <label for="password" class="block text-[10px] font-bold text-text-dim uppercase tracking-widest">Password</label>
          ...
          <div class="input-icon-wrapper">
            ...
            <input type="password" id="password" bind:value={password} placeholder="••••••••" required class="input input-has-icon-left" />
          </div>
        </div>

        <button type="submit" class="btn btn-primary w-full mt-2 py-4" disabled={isLoading}>
          {#if isLoading}
            Signing in...
          {:else}
            Sign In
      ...
      <p class="text-center mt-6 text-text-dim text-[10px] font-bold uppercase tracking-tight">
        New here? <a href={signupUrl} class="text-accent hover:text-accent-hover underline underline-offset-4">Create account</a>
      </p>
    </div>
  </div>
</div>
