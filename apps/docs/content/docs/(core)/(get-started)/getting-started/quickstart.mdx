...
1. Log in to the dashboard at [app.owostack.com](https://app.owostack.com) and click **Create Organization**.
2. Name it anything you like (e.g. "My Tutorial App").
3. Go to **Settings → API Keys → Create New Key**. Name it "Tutorial", choose the **Sandbox** environment, and copy the key. Sandbox keys start with `owo_sk_test_` and only ever talk to `sandbox.owostack.com`, so nothing in this tutorial can touch production (see [Environments](/getting-started/environments)).
...
<Callout type="warning">
  Without webhooks, Owostack cannot confirm payments. Every step after this
  depends on webhooks working.
</Callout>
