...
### Sandbox accounts are managed by Owostack

In the sandbox you don't need any of the above. Owostack keeps a shared test account on each supported provider and your organization uses them automatically — they show up as **Managed by Owostack** in **Settings → Providers**. A sandbox account you add yourself for a provider overrides the managed one for that provider.

Live always uses your own accounts. See [Environments](/getting-started/environments#sandbox-is-managed).
...
4. **Fallback** — the first configured provider account (in the sandbox this includes the managed accounts)
...
- Bachs — Bachs keeps the card for renewals but exposes no off-session charge API, so Owostack never auto-charges a Bachs customer. Overage and invoice payments fall back to a hosted checkout link (`POST /billing/invoice/{id}/pay` returns `checkoutUrl`)
...
- Bachs — product-centric for plans and credit packs; supports product-less raw-amount checkouts for invoices. Amounts are decimal strings on the wire, converted from Owostack minor units by the adapter
...
- Bachs — native subscription updates with `proration_behavior: invoice_now`. Bachs requires the target product to bill at the same interval; if it rejects the change Owostack falls back to its non-native path. A plan interval change on Bachs creates a replacement product (billing cycles are immutable) and archives the old one.
...
4. Configure a provider account (and, for the sandbox, add Owostack's test credentials for it to the managed set)
