...
<Callout type="info">
  **Sandbox needs no setup.** Sandbox payments run on provider accounts that
  Owostack manages, and their webhooks already point at Owostack. The steps
...
In your provider dashboard, paste the webhook URL and save. If you brought your own sandbox account, configure it on the provider's test environment and use `https://sandbox.owostack.com` as the host instead.
...
2. Confirm the provider account environment matches your test/live setup, and that the host matches (`api.owostack.com` for live, `sandbox.owostack.com` for your own sandbox account)
