---
title: customer()
description: Create, resolve, look up, list, and configure customers
draft: false
icon: User
---
...
Every read on this namespace (`get()`, `usageHistory()`, and the setters) accepts the **same identifier you use in `track()` / `check()`**: your own external customer ID, the customer's email, or the internal Owostack ID.
...
const customer = await owo.customer.get(customer: string);

const page = await owo.customer.list({
...
### `owo.customer.get(customer)`

| Parameter  | Type     | Required | Description                                                                 |
| ---------- | -------- | -------- | --------------------------------------------------------------------------- |
| `customer` | `string` | ✅       | External customer ID (what you pass to `track`), email, or internal ID     |

Returns the same `CustomerResult` as `owo.customer()`. Throws `OwostackError` with `Customer not found` when nothing matches within your organization.

### `owo.customer.list(params?)`
...
Use [`owo.billing.usage()`](/sdk/billing) when you want uninvoiced overage and estimated billable amounts.

## Returns

`owo.customer()`, `get()`, `setFeatureConfig()`, and `setOverageLimit()` all return the customer plus the current customer billing config.

```ts
interface CustomerResult {
  id: string;                 // Internal Owostack ID
...
`owo.customer.list()` returns a page of lightweight customer summaries (no billing config):
...
await owo.track({ customer: "workspace_alpha", feature: "agent_turns" });

const customer = await owo.customer.get("workspace_alpha");
console.log(customer.id);         // internal UUID
console.log(customer.externalId); // "workspace_alpha"

// Email works as well
const byEmail = await owo.customer.get("billing@acme.com");
```
...
const page = await owo.customer.list({ limit: 25 });

// Search across email, name and external id
const matches = await owo.customer.list({ search: "acme" });

// Exact lookups
const { data } = await owo.customer.list({ externalId: "workspace_alpha" });

// Walk every page
for (let offset = 0; ; offset += 100) {
  const { data, total } = await owo.customer.list({ limit: 100, offset });
