...
When a renewal charge fails, the provider marks the subscription `past_due` and retries the card over the following days, emailing the customer. Owostack follows the provider: the plan's entitlements keep flowing while the subscription is `past_due`, and access is revoked when the provider ends it (`canceled` / `unpaid`) — or reactivated when a retry succeeds. During dunning `check()` and `track()` still return `allowed: true`, and `details` tells you why:
