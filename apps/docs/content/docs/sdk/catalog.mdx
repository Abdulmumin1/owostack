---
title: Catalog Builders
description: "Reference: metered(), boolean(), plan() — define features and plans as code"
icon: Blocks
---
...
apiCalls.limit(50000, { overage: "charge", overagePrice: 100, maxOverageUnits: 100000 })
```
...
```ts
apiCalls.config({ limit: 50000, reset: "monthly", overage: "charge", overagePrice: 100 })
```
...
```ts
const result = await apiCalls.check("user@example.com");
const result = await apiCalls.check("user@example.com", { value: 5 });
const result = await apiCalls.check("user@example.com", { sendEvent: true });
```
...
```ts
await apiCalls.track("user@example.com");
await apiCalls.track("user@example.com", 5);
await apiCalls.track("user@example.com", 1, { entity: "workspace_123" });
```
...
| `config.features` | `PlanFeatureEntry[]` | Yes | Array of feature entries from `.limit()`, `.on()`, etc. |
...
Push the catalog to the API. Creates and updates features and plans.
...
export default new Owostack({
  ...
  catalog: [
    plan("starter", {
      name: "Starter", price: 0, currency: "NGN", interval: "monthly",
      features: [apiCalls.limit(1000), analytics.off()],
    }),
    plan("pro", {
      name: "Pro", price: 500000, currency: "NGN", interval: "monthly",
      features: [apiCalls.limit(50000), analytics.on()],
    }),
  ],
});
```
